...
function drawTable(data: Data, sortBy: SortBy): string {
  ...
  const heading = Array.from({ length: keys.length }, (_, i) =>
  ...
  const generateRow = (
    ...
    return (
      ...
      lengths
        .map(
          (len, i) =>
            ' '.repeat(PADDING) + cols[i].toString().padEnd(len + PADDING, ' '),
        )
        .join('|') +
      '|'
    );
  ...
  const mainContain = sortedData
    .map((data) => generateRow(Object.values(data), lengths))
    .join('\n');
